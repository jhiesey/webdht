...
Refcounting:
* muxrpc indicates end, but not for arbitrary streams
...
const wrtcModules = [{
	...
	module: 'electron-webrtc',
	setup: function (module) {
		return module()
	}
}]

for (let i = 0; i < wrtcModules.length; i++) {
	try {
		let w = require(wrtcModules[i].module)
		const setup = wrtcModules[i].setup
		if (setup)
			w = setup(w)
...
/*
When opening muxed stream, advertise/check maximum data transfer

On upgrade, exchange websocket urls in addition to ice candidates

Need a way to verify/prefer diversity of connections

Need a scoring function for connections and an algorithm that approximates
optimal
*/
...
var Connection = function (connector, stream, peerOpts, opts) {
	...
	self._origId = opts.id
	self._introduced = opts.introduced
...
Connection.prototype.openStream = function (name, onclose) {
	var self = this
	// const streamName = 'app:' + name
	// self._refStream(streamName)
	onclose = onclose || noop

	return self._handle.openStream(name, onclose)
}
...
Connector.prototype._onConnectFrom = function (relay, id) {
	...
	let duplex = PullStreamEnd(function (err) {
		relay.emit('relay-end', id)
	})
...
Connector.prototype._onConnectTo = function (from, id) {
	...
	// TODO: ref streams
	console.log('MAKING CONNECTFROM CALL')
	from.emit('relay', to.id)
	to.emit('relay', from.id)
	return to._handle.connectFrom(from.id, function () {
		console.log('CONNECTFROM CALLBACK')
		from.emit('relay-end', to.id)
		to.emit('relay-end', from.id)
	})
}
...
Connector.prototype.connectTo = function (descriptor, cb) {
	...
	if (descriptor.url) {
		...
		descriptor.relay.emit('relay', id)
		let stream = descriptor.relay._handle.connectTo(id, function (err) {
			descriptor.relay.emit('relay-end', id)
		})
		self._onStreamOpen(stream, {
			initiator: true,
			introduced: true,
